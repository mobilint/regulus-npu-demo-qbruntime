...
mobilint::post_hdmi::YOLOv8PosePostProcessor::YOLOv8PosePostProcessor()
    : YOLOv8PostProcessor() {
...
*/
void mobilint::post_hdmi::YOLOv8PosePostProcessor::decode_extra(
...
void mobilint::post_hdmi::YOLOv8PosePostProcessor::run_postprocess(
    const std::vector<std::vector<float>>& npu_outs) {
    double start = set_timer();

    if (npu_outs.size() / 3 != m_nl) {  // 3 keypoints + 3 dets
        throw std::invalid_argument(
            "YOLOv8 Pose post-processing is "
            "expected to receive 9 NPU outputs, however received " +
...
*/
void mobilint::post_hdmi::YOLOv8PosePostProcessor::plot_keypoints(
...
*/
void mobilint::post_hdmi::YOLOv8PosePostProcessor::plot_extras(
    cv::Mat& im, std::vector<std::vector<float>>& extras) {
    plot_keypoints(im, extras);
}

void mobilint::post_hdmi::YOLOv8PosePostProcessor::worker() {
