...
mobilint::post_lcd::YOLOv8PostProcessor::YOLOv8PostProcessor() {
    ...
    ticket = 0;
    destroyed = false;
    mThread = std::thread(&mobilint::post_lcd::YOLOv8PostProcessor::worker, this);
}

mobilint::post_lcd::YOLOv8PostProcessor::YOLOv8PostProcessor(int nc, int imh, int imw,
                                                             float conf_thres,
                                                             float iou_thres,
                                                             bool verbose) {
...
*/
std::vector<int> mobilint::post_lcd::YOLOv8PostProcessor::generate_strides(int nl) {
...
*/
std::vector<std::vector<int>> mobilint::post_lcd::YOLOv8PostProcessor::generate_grids(
    int imh, int imw, const std::vector<int>& strides) {
...
int mobilint::post_lcd::YOLOv8PostProcessor::get_nl() const { return m_nl; }

int mobilint::post_lcd::YOLOv8PostProcessor::get_nc() const { return m_nc; }

/*
        Return the type of Post Processing
*/
mobilint::post_lcd::PostType mobilint::post_lcd::YOLOv8PostProcessor::getType() const {
    return mType;
}
...
*/
float mobilint::post_lcd::YOLOv8PostProcessor::sigmoid(float num) {
...
*/
float mobilint::post_lcd::YOLOv8PostProcessor::inverse_sigmoid(float num) {
    return -log(1 / num - 1);
}
...
*/
std::vector<float> mobilint::post_lcd::YOLOv8PostProcessor::softmax(
...
*/
float mobilint::post_lcd::YOLOv8PostProcessor::area(float xmin, float ymin, float xmax,
                                                    float ymax) {
...
*/
float mobilint::post_lcd::YOLOv8PostProcessor::get_iou(const std::array<float, 4>& box1,
...
*/
void mobilint::post_lcd::YOLOv8PostProcessor::xywh2xyxy(
...
*/
void mobilint::post_lcd::YOLOv8PostProcessor::decode_boxes(
    const std::vector<float>& npu_out, const std::vector<int>& grid, int stride, int idx,
...
*/
void mobilint::post_lcd::YOLOv8PostProcessor::decode_extra(
...
*/
void mobilint::post_lcd::YOLOv8PostProcessor::decode_conf_thres(
...
#pragma omp parallel for num_threads(mOpenmpThreadCount * 2) \
    ...
    for (int i = 0; i < grid_h * grid_w; i++) {
        ...
        for (int j = 0; j < m_nc; j++) {
            if (npu_out[i * tmp_no + 64 + j] > m_inverse_conf_thres) {
...
/*
        Apply NMS
*/
void mobilint::post_lcd::YOLOv8PostProcessor::nms(
...
*/
double mobilint::post_lcd::YOLOv8PostProcessor::set_timer() {
...
mobilint::post_lcd::YOLOv8PostProcessor::get_result_box() {
    return final_boxes;
}

std::vector<float>& mobilint::post_lcd::YOLOv8PostProcessor::get_result_score() {
    return final_scores;
}

std::vector<int>& mobilint::post_lcd::YOLOv8PostProcessor::get_result_label() {
    return final_labels;
}

std::vector<std::vector<float>>&
mobilint::post_lcd::YOLOv8PostProcessor::get_result_extra() {
    return final_extra;
}

void mobilint::post_lcd::YOLOv8PostProcessor::run_postprocess(
...
*/
void mobilint::post_lcd::YOLOv8PostProcessor::compute_ratio_pads(cv::Mat im, float& ratio,
                                                                 float& xpad,
                                                                 float& ypad) {
...
*/
void mobilint::post_lcd::YOLOv8PostProcessor::plot_boxes(
...
*/
void mobilint::post_lcd::YOLOv8PostProcessor::plot_extras(
...
*/
void mobilint::post_lcd::YOLOv8PostProcessor::print(std::string msg) {
#ifdef DEBUG
...
void mobilint::post_lcd::YOLOv8PostProcessor::compute_ratio(int srcHeight, int srcWidth,
                                                            float& h_ratio,
                                                            float& w_ratio) {
    h_ratio = (float)m_imh / srcHeight;
    w_ratio = (float)m_imw / srcWidth;
}

void mobilint::post_lcd::YOLOv8PostProcessor::compute_ratio_pads(int srcHeight,
                                                                 int srcWidth,
                                                                 float& ratio, int& xpad,
                                                                 int& ypad) {
    float w_ratio = (float)m_imw / srcWidth;
    float h_ratio = (float)m_imh / srcHeight;
    if (w_ratio > h_ratio) {
        ratio = h_ratio;
        ypad = 0;
        xpad = (int)(m_imw - (int)floor(ratio * srcWidth + 0.5)) / 2;
    } else {
        ratio = w_ratio;
        xpad = 0;
        ypad = (int)(m_imh - (int)floor(ratio * srcHeight + 0.5)) / 2;
    }
}

void mobilint::post_lcd::YOLOv8PostProcessor::rescaleImg(
    std::vector<std::array<int, 4>>& rescaled_boxes, int srcHeight, int srcWidth) {
    float h_ratio, w_ratio;
    compute_ratio(srcHeight, srcWidth, h_ratio, w_ratio);

    for (int i = 0; i < final_boxes.size(); i++) {
        int xmin = (int)(final_boxes[i][0]) / w_ratio;
        int ymin = (int)(final_boxes[i][1]) / h_ratio;
        int xmax = (int)(final_boxes[i][2]) / w_ratio;
        int ymax = (int)(final_boxes[i][3]) / h_ratio;

        xmin = std::max(0, xmin);
        ymin = std::max(0, ymin);
        xmax = std::min(srcWidth, xmax);
        ymax = std::min(srcHeight, ymax);

        int width = xmax - xmin;
        int height = ymax - ymin;

        std::array<int, 4> tmp = {xmin, ymin, width, height};
        rescaled_boxes.push_back(tmp);
    }
}

void mobilint::post_lcd::YOLOv8PostProcessor::rescaleImgWithPadding(
    std::vector<std::array<float, 4>>& rescaled_boxes, int srcHeight, int srcWidth) {
    float ratio;
    int xpad, ypad;
    compute_ratio_pads(srcHeight, srcWidth, ratio, xpad, ypad);

    for (int i = 0; i < final_boxes.size(); i++) {
        float xmin = (float)(final_boxes[i][0] - xpad) / ratio;
        float ymin = (float)(final_boxes[i][1] - ypad) / ratio;
        float xmax = (float)(final_boxes[i][2] - xpad) / ratio;
        float ymax = (float)(final_boxes[i][3] - ypad) / ratio;

        xmin = std::max((float)0, xmin);
        ymin = std::max((float)0, ymin);
        xmax = std::min((float)srcWidth, xmax);
        ymax = std::min((float)srcHeight, ymax);

        float width = xmax - xmin;
        float height = ymax - ymin;

        std::array<float, 4> tmp = {xmin, ymin, width, height};
        rescaled_boxes.push_back(tmp);
    }
}

void mobilint::post_lcd::YOLOv8PostProcessor::rescaleImgWithPaddingV2(
    std::vector<std::array<float, 4>>& rescaled_boxes, int srcHeight, int srcWidth) {
    float gain;
    int xpad, ypad;

    gain = std::min((float)m_imh / srcHeight, (float)m_imw / srcWidth);

    xpad = (int)std::round(((float)m_imw - srcWidth * gain) / 2.0f - 0.1f);
    ypad = (int)std::round(((float)m_imh - srcHeight * gain) / 2.0f - 0.1f);

    for (int i = 0; i < final_boxes.size(); i++) {
        float xmin = (float)(final_boxes[i][0] - xpad) / gain;
        float ymin = (float)(final_boxes[i][1] - ypad) / gain;
        float xmax = (float)(final_boxes[i][2] - xpad) / gain;
        float ymax = (float)(final_boxes[i][3] - ypad) / gain;

        xmin = std::max((float)0, std::min(xmin, (float)srcWidth));
        xmax = std::max((float)0, std::min(xmax, (float)srcWidth));
        ymin = std::max((float)0, std::min(ymin, (float)srcHeight));
        ymax = std::max((float)0, std::min(ymax, (float)srcHeight));
...
void mobilint::post_lcd::YOLOv8PostProcessor::worker() {
...
uint64_t mobilint::post_lcd::YOLOv8PostProcessor::enqueue(
...
void mobilint::post_lcd::YOLOv8PostProcessor::receive(uint64_t receipt_no) {
    // auto thres_id = std::hash<std::thread::id>{}(std::this_thread::get_id());
