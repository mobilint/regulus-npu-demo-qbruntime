...
#include "src/postprocess/yolo_anchor_seg_post.h"
...
void inference_seg_anchor(std::string mxq_path, std::string img_path,
...
void inference_seg_anchorless(std::string mxq_path, std::string img_path,
                              std::string save_path) {
    NPUModel model(mxq_path);
...
int main(int argc, char** argv) {
    ...
    if (contains(model_name, "yolov5") || contains(model_name, "yolov7")) {
        inference_seg_anchor(mxq_path, img_path, save_path);
    } else if (contains(model_name, "yolo")) {
        inference_seg_anchorless(mxq_path, img_path, save_path);
    } else {
